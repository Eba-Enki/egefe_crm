...
'ham-sayimlar':'<strong>SHEET & STRİP STOK SAYIM LİSTESİ</strong>',
'ham-sayim':'Fiziki Stok Sayımı',
...
function showPage(id,skipRender){
  ...
  if(currentPortal==='stok'){
    ...
    var _shss=document.getElementById('topbar-stok-ham-sayim-sablon');if(_shss)_shss.style.display=sbm.hamSayim?'':'none';
    ...
    var _sbss=document.getElementById('topbar-stok-bitmis-sayim-sablon');if(_sbss)_sbss.style.display=sbm.bmSayim?'':'none';
    var _spm=document.getElementById('topbar-stok-param');   if(_spm)_spm.style.display=(sbm.param&&(isAdmin||canWrite))?'':'none';
  } else {
    ['topbar-stok-excel','topbar-stok-ham-giris','topbar-stok-ham-cikis','topbar-stok-ham-sayim','topbar-stok-ham-sayim-sablon','topbar-stok-bitmis-giris','topbar-stok-bitmis-cikis','topbar-stok-bitmis-sayim','topbar-stok-bitmis-sayim-sablon','topbar-stok-param'].forEach(function(bid){var b=document.getElementById(bid);if(b)b.style.display='none';});
  }
...
function goTeklifForm(editId,servisId){
  ...
  if(editId){
    ...
    var _imzaLbl=document.getElementById('tf-imza-gizle-label');if(_imzaLbl)_imzaLbl.style.display=currentPortal==='satis'?'flex':'none';
    var _imzaCb=document.getElementById('tf-imza-gizle');if(_imzaCb)_imzaCb.checked=false;
    ...
    var _imzaLbl0=document.getElementById('tf-imza-gizle-label');if(_imzaLbl0)_imzaLbl0.style.display=currentPortal==='satis'?'flex':'none';
...
function goMusteriForm(editId){
  ...
  var mfPage=document.getElementById('mf-form-page');if(mfPage)mfPage.style.maxWidth=currentPortal==='satis'?'820px':'580px';
...
function openTiCombo(idx){
  ...
  const sorted=isServis
    ...
    :[...filtered].sort((a,b)=>kategoriSiraIndex(a.kategori)-kategoriSiraIndex(b.kategori)||(a.kategori||'').localeCompare(b.kategori||'','tr'));
...
function selectTiUrun(idx, itemIdx){
  ...
  const aciklama=isServis&&modelStr?urunAdi+' — '+modelStr:(kategori?urunAdi+' ('+kategori+')':urunAdi);
