...
function renderMusteriler(){
  ...
  tbody.innerHTML=pagedM.map(m=>{return`<tr><td><span class="kn-badge" style="color:var(--accent);font-size:10px">${esc(m.kayitNo||'—')}</span></td><td style="font-weight:500">${esc(m.kurum)}</td><td style="color:var(--text2)">${esc(m.kisi||'—')}</td><td class="td-mono">${esc(m.tel||'—')}</td><td style="color:var(--text2)">${esc(m.email||'—')}</td><td>${esc(m.sehir||'—')}</td><td><div class="action-row" style="justify-content:flex-end"><button class="btn-icon" onclick="goMusteriForm('${esc(m.id)}')"><img src="icons/edit_icon.png" alt="Düzenle" style="width:14px;height:14px;display:block"></button><button class="btn-icon" style="color:var(--red)" onclick="confirmDelete('musteri','${esc(m.id)}')"><img src="icons/delete.png" alt="Sil" style="width:14px;height:14px;display:block"></button></div></td></tr>`;}).join('');
}
...
function renderUrunler(){
  ...
  tbody.innerHTML=pagedU.map(u=>{return`<tr><td class="td-mono" style="color:var(--accent);font-size:11px">${esc(u.urunKodu||'—')}</td><td style="font-weight:500">${esc(u.urunAdi)}</td><td style="color:var(--text2)">${esc(u.marka||'—')}</td>${isSatis?`<td style="color:var(--text2);font-size:12px">${esc(u.kategori||'—')}</td>`:''}<td class="td-mono" style="color:var(--text2)">${esc(u.model||'—')}</td><td class="td-mono" style="color:var(--amber)">${_fmtFiyat(u.fiyat,u.paraBirimi)}</td><td><div class="action-row" style="justify-content:flex-end"><button class="btn-icon" onclick="goUrunForm('${esc(u.id)}')"><img src="icons/edit_icon.png" alt="Düzenle" style="width:14px;height:14px;display:block"></button><button class="btn-icon" style="color:var(--red)" onclick="confirmDelete('urun','${esc(u.id)}')"><img src="icons/delete.png" alt="Sil" style="width:14px;height:14px;display:block"></button></div></td></tr>`;}).join('');
}
