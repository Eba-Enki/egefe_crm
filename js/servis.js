...
function openServisDetay(id){
  ...
  document.getElementById('sd-kayitno').textContent=s.kayitNo;
  document.getElementById('sd-durum').innerHTML=durumBadge(s.durum);
  ...
  document.getElementById('sd-body').innerHTML=`
    ...
    <div class="pd-section">
      ...
      <div class="pd-info-grid">
        <div class="pd-info-box"><div class="info-item-label">Marka</div><div class="info-item-val">${esc(s.marka||'—')}</div></div>
        <div class="pd-info-box"><div class="info-item-label">Model</div><div class="info-item-val">${esc(s.model||'—')}</div></div>
...
function renderTable(){
  ...
  tbody.innerHTML=pagedServis.map(s=>{
    ...
    return`<tr style="cursor:pointer${isArsiv?';opacity:0.8':''}" onclick="openServisDetay('${s.id}')">
      ...
      <td style="text-align:center"><span class="kn-badge" style="border:none;background:transparent;padding:0">${esc(s.kayitNo)}</span></td>
      <td class="td-mono" style="color:var(--text2);text-align:center">${fmtDate(s.gelisTarihi)}</td>
      <td style="font-weight:500;max-width:220px;white-space:normal;word-break:break-word">${esc(s.kurumAdi||'—')}</td>
      <td class="td-mono" style="text-align:center">${esc(s.seriNo||'—')}</td>
      <td style="text-align:center">${durumBadge(s.durum)}</td>
      <td style="text-align:center"><span class="badge ${s.garantiDurumu==='Evet'?'badge-garanti-evet':'badge-garanti-hayir'}">${esc(s.garantiDurumu)}</span></td>
      <td style="color:var(--text2);font-size:12px;text-align:center">${esc(s.ilgiliKisi||'—')}</td>
