...
switch ($method) {
    ...
    case 'DELETE':
        ...
        $evrakStmt = $pdo->prepare('SELECT DISTINCT duzeltme_evrak_no FROM finished_stock_count_items WHERE count_id = ? AND duzeltme_evrak_no IS NOT NULL');
        ...
        try {
            foreach ($girisIds as $girisId) {
                $pdo->prepare('DELETE FROM finished_stock_lots WHERE giris_id = ?')->execute([$girisId]);
                $pdo->prepare('DELETE FROM finished_stock_entries WHERE id = ?')->execute([$girisId]);
            }
            foreach ($cikisIds as $cikisId) {
                ...
                foreach ($items->fetchAll() as $item) {
                    if ($item['lot_id']) $incStmt->execute([$item['miktar_cikis'], $item['lot_id']]);
                }
                $pdo->prepare('DELETE FROM finished_stock_exits WHERE id = ?')->execute([$cikisId]);
            }
